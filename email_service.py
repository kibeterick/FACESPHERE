...
Generated by AKIRA AI System'''
    ...
    def send_email(self, to_email: str, subject: str, body: str, 
        ...
        if self.email_address == '[email]' or self.email_password == '[password]':
            return {
                'success': False,
                'message': 'Gmail not configured. Set EMAIL_ADDRESS and EMAIL_PASSWORD in .env',
        ...
        try:
            ...
            # Try to send via real SMTP - try multiple ports
            ports_to_try = [
                (self.smtp_port, 'TLS'),  # 587
                (465, 'SSL'),              # Alternative
            ]
            ...
            for port, method in ports_to_try:
                try:
                    print(f"🔄 Trying {self.smtp_server}:{port} ({method})...")
                    ...
                    print(f"   Via: Gmail SMTP (port {port})")
                    ...
                    return {
                        'success': True,
                        'message': f'Email sent successfully via Gmail SMTP (port {port})',
            ...
            if last_error:
                if isinstance(last_error, smtplib.SMTPAuthenticationError):
                    error_msg = f"Gmail authentication failed: {str(last_error)}"
                    print(f"❌ {error_msg}")
                    ...
                    return {
                        ...
                        'solution': 'Verify App Password at https://myaccount.google.com/apppasswords'
                    }
                else:
                    error_msg = "SMTP ports blocked by firewall/network"
                    print(f"❌ {error_msg}")
                    print("   Tried ports: 587, 465")
                    print("   Solution: Check firewall or use Gmail API")
                    
                    return {
                        'success': False,
                        'message': error_msg,
                        'error_type': 'network_blocked',
                        'solution': 'Read EMAIL_SENDING_SOLUTIONS.md for alternatives'
                    }
